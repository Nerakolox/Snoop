...
//
// 批次 3 起本组件不再直接嵌在设置页，而是作为「AI」页的 body 常驻，
// 由 `tab` prop 决定展示哪个区块（服务 / 隐私 / 分类 / 记录）。
// 所有 state 保持在组件内不变——切 tab 只改 prop，数据不丢。
...
export type AiTab = "service" | "privacy" | "category" | "log";

export default function AISettings({ tab }: { tab: AiTab }) {
    ...
    <>
      ...
      {tab === "service" && (
        <div className="settings-group">
          <div className="settings-group-title">服务</div>
      ...
      {tab === "privacy" && (
        <>
          <div className="settings-group">
            <div className="settings-group-title">数据层级</div>
      ...
      {tab === "category" && (
        <div className="settings-group">
          <div className="settings-group-title">分类</div>
      ...
      {tab === "log" && (
        <div className="settings-group">
          <div className="settings-group-title">记录</div>
